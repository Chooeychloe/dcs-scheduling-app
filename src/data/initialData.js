
export const initialRooms = ['Rm 303', 'Rm 305', 'Rm 107', 'Rm 109', 'Audi', 'Faculty Rm', 'Accre Rm', 'CL1', 'CL2', 'CL3', 'CL4', 'CL5',];

export const initialDays = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];


export const initialSubjects = {
    'IT': {
        '1st Semester': [
            {
                '1st Year': [
                    'Ethics',
                    'Purposive Communication',
                    'Kontekstwalisadong Komunikasyon sa Filipino',
                    'Discrete Structure',
                    'Introduction to Computing',
                    'Computer Programming 1',
                    'Movement Enhancement',
                    'National Service Training Program 1',
                    'Institutional Orientation'
                ],
                '2nd Year': [
                    'Mga Babasahin Hinggil sa Kasaysayan ng Pilipinas',
                    'The Contemporary World',
                    'Gender and Society',
                    'Panitikang Panlipunan',
                    'Information Management',
                    'Platform Technologies',
                    'Object Oriented Programming',
                    'Physical Activities towards Health and Fitness I',
                ],
                '3rd Year': [
                    'Introduction to Human Computer Interaction',
                    'Information Assurance and Security 1',
                    'Network Fundamentals',
                    'System Analysis and Design',
                    'Application Development and Emerging Technologies',
                    'Methods of Research',
                ],
                '4th Year': [
                    'Social and Professional Issues',
                    'IT ELECTIVE 3 (Integrated Programming and Technologies 2)',
                    'IT ELECTIVE 4 (Systems Integration and Architecture 2)',
                    'Systems Administration and Maintenance',
                    'Capstone Project and Research 2',
                ]
            }
        ],

        '2nd Semester': [
            {
                '1st Year': [
                    'Arts Appreciation',
                    'Science, Technology, and Society',
                    'Dalumat Ng/Sa Filipino',
                    'Mathematics in the Modern World',
                    'Computer Programming 2',
                    'Web System and Technologies 1',
                    'Fitness Exercise',
                    'National Service Training Program 2'
                ],
                '2nd Year': [
                    'Understanding the Self',
                    'Data Structures and Algorithms',
                    'Integrated Programming and Technologies 1',
                    'Open Source Technology',
                    'Advanced Database System',
                    'Multimedia Systems',
                    'Physical Activities towards Health and Fitness II',
                ],
                '3rd Year': [
                    'Rizal: Life, Works, and Writings',
                    'Quantitative Methods (Modeling & Simulation)',
                    'IT ELECTIVE 1 (Human Computer Interaction 2)',
                    'IT ELECTIVE 2 (Web System and Technologies 2)',
                    'Information Assurance and Security 2',
                    'Network Management',
                    'Capstone Project and Research 1'
                ],
                '4th Year': [
                    'Practicum'
                ]
            }
        ],
        'Midyear': ['Applied Statistics',
            'System Integation and Architecture 1'],

    },
    'CS': {
        '1st Semester': [
            {
                '1st Year': [
                    'Ethics',
                    'Purposive Communication',
                    'Kontekstwalisadong Komunikasyon sa Filipino',
                    'Discrete Structure I',
                    'Introduction to Computing',
                    'Computer Programming 1',
                    'Movement Enhancement',
                    'National Service Training Program 1',
                    'Institutional Orientation',
                ],
                '2nd Year': [
                    'Mga Babasahin Hinggil sa Kasaysayan ng Pilipinas',
                    'Analytic Geometry',
                    'Discrete Structures II',
                    'Digital Logic Design',
                    'Object Oriented Programming',
                    'Information Management',
                    'Fundamentals of Inforamtion Systems',
                    'Physical Activities towards Health and Fitness I',
                ],
                '3rd Year': [
                    'Linear Algebra',
                    'Software Engineering II',
                    'Operating Systems',
                    'Networks and Communication',
                    'CS Elective 1 (Computer Graphics and Visual Computing)',
                    'Applications Dev`t and Emerging Technologies',
                    'Social and Professional Issues',

                ],
                '4th Year': [
                    'Human Computer Interaction',
                    'Automata Theory and Formal Languages',
                    'Intelligent Systems',
                    'CS Elective 3 (Internet of Things)',
                    'Undergraduate Thesis I',

                ]
            }
        ],

        '2nd Semester': [
            {
                '1st Year': [
                    'Arts Appreciation',
                    'Mathematics in the Modern World',
                    'Science, Technology, and Society',
                    'Dalumat Ng/Sa Filipino',
                    'Computer Programming 2',
                    'Web System and Technologies',
                    'Fitness Exercise',
                    'National Service Training Program 2'
                ],
                '2nd Year': [
                    'Understanding the Self',
                    'Panitikang Panlipunan',
                    'Calculus',
                    'Architecture and Organization',
                    'Software Engineering I',
                    'Data Structures and Algorithms',
                    'Adavance Database Management System',
                    'Physical Activities towards Health and Fitness 2',
                ],

                '3rd Year': [
                    'Life and Works of Rizal',
                    'Experimental Statistics',
                    'Design and Analysis of Algorithm',
                    'CS Elective 2 (Introduction to Game Development)',
                    'Methods of Research',
                    'Information Assurance and Security'
                ],
                '4th Year': [
                    'The Contemporary World',
                    'Gender and Society',
                    'Numerical and Symbolic Computation',
                    'Undergraduate Thesis II'
                ]
            }
        ],
        'Midyear': [
            'Practicum'
        ]

    }
};

export const initialSections = {
    'IT': {
        '1st Year': ['IT 1A', 'IT 1B'],
        '2nd Year': ['IT 2A', 'IT 2B'],
        '3rd Year': ['IT 3A'],
        '4th Year': ['IT 4A']
    },
    'CS': {
        '1st Year': ['CS 1A', 'CS 1B'],
        '2nd Year': ['CS 2A'],
        '3rd Year': ['CS 3A', 'CS 3B'],
        '4th Year': ['CS 4A']
    }
};


export const initialFaculty = [
    'Edan A. Belgica',
    'Clarissa Rostrollo',
    'Clarence Salvador',
    'Alvina Ramallosa',
    'Alvin Catalo'
];
